"""Train the compact Phase 4 JEPA completion model."""
...
def evaluate_epoch(model: JEPACompletionModel, loader: torch.utils.data.DataLoader, device: torch.device, variant: str, lambda_recon: float) -> dict[str, float]:
    model.eval()
    ...
    for batch in loader:
        ...
        latent_loss = jepa_loss(outputs["z_pred"], outputs["z_target"])
        reconstruction = masked_reconstruction_bce(outputs["logits"], target, mask)
        total = latent_loss + lambda_recon * reconstruction if variant == "jepa_reconstruction" else latent_loss
        size = inputs.shape[0]
        totals["jepa_loss"] += float(latent_loss.item()) * size
        ...
        count += size
    latent_metrics = latent_variance_metrics(torch.cat(context_values), torch.cat(target_values), torch.cat(pred_values))
    return {name: value / count for name, value in totals.items()} | latent_metrics | {"collapse_flag": float(latent_metrics["context_mean_std"] < 1e-4 or latent_metrics["target_mean_std"] < 1e-4 or latent_metrics["pred_mean_std"] < 1e-4)}
...
def main() -> None:
    ...
    parser.add_argument("--variant", choices=("pure_jepa", "jepa_reconstruction"), default="jepa_reconstruction")
    ...
    if args.variant == "pure_jepa" and args.lambda_recon != 0.1:
    ...
    model = JEPACompletionModel(args.latent_dim, args.predictor_hidden_dim, args.ema_decay).to(device)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch in loaders["train"]:
            ...
            latent_loss = jepa_loss(outputs["z_pred"], outputs["z_target"])
            reconstruction = masked_reconstruction_bce(outputs["logits"], target, mask)
            total = latent_loss + args.lambda_recon * reconstruction if args.variant == "jepa_reconstruction" else latent_loss
            total.backward()
            optimizer.step()
            ...
            totals["total_loss"] += float(total.item()) * size
            items += size
        validation = evaluate_epoch(model, loaders["val"], device, args.variant, args.lambda_recon)
    ...
    config = {
        "phase": "4", "subset_root": str(args.subset_root), "split": {name: len(loader.dataset) for name, loader in loaders.items()},
        "mask_type": args.mask_type, "missing_ratio": args.missing_ratio, "mask_seed": args.seed,
        "variant": args.variant, "lambda_recon": args.lambda_recon, "latent_dim": args.latent_dim, "predictor_hidden_dim": args.predictor_hidden_dim, "ema_decay": args.ema_decay,
        "model": "JEPACompletionModel", "model_parameter_count_total": sum(parameter.numel() for parameter in model.parameters()), "trainable_parameter_count": sum(parameter.numel() for parameter in model.trainable_parameters()),
